...
export const EmailIDVerification = ({ token }) => {
  ...
  const result = loading ? (
    ...
    <Fragment>Email ID Verification Successful</Fragment>
  ) : (
    <Fragment>
      {resendOTP.isError ? (
      ...
      <Typography variant="subtitle2">
        address <b>{requestOTP.data?.email}</b>
      </Typography>
      <TextField
        autoFocus
        margin="dense"
...
export const EmailIDVerificationWrapper = () => {
  ...
  return verifyToken.isFetching || verifyToken.isLoading ? (
    ...
    <EmailIDVerification token={token} />
  );
};
